...
export default class AllRequest extends Component{
    ...
    retrieveLecturers(){
        axios.get("http://localhost:8070/instructors").then(res =>{
            if(res.data.success){
                this.setState({
                    lecturers:res.data.existingInstructors
                });
    ...
    onDelete = (id) =>{
        axios.delete(`http://localhost:8070/instructors/delete/${id}`).then((res) =>{
            ...
            <div className="container">
                <div className="row">
                    <div className="col-lg-9 mt-2 mb-2">
                        <h4 className='header-name'>All Instructor Requests</h4>
                    </div>
                        ...
                        {this.state.lecturers.map((lecturers,index) =>(
                            <tr key={index}>
                                <th scope="row">{index+1}</th>
                                    ...
                                    <td>
                                        <a className="btn btn-warning" href={`/dashboard/editrequest/${lecturers._id}`}>
