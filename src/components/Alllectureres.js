...
export default class Alllectureres extends Component{
  constructor(props){
    super(props);
  ...
  retrieveInstructors(){
    axios.get("http://localhost:8070/lecturers").then(res =>{
      if(res.data.success){
        this.setState({
          instructors:res.data.existingLecturers
        });
      ...
      <div className="container">
        <div className="row">
          <div className="col-lg-9 mt-2 mb-2">
            <h4 className='header-name'>All Lecturers</h4>
          </div>
          ...
          <tbody>
            {this.state.instructors.map((instructors,index) =>(
              <tr key={index}>
                ...
                <td>
                  <a className="btn btn-warning" href={`/dashboard/editlecture/${instructors._id}`}>
        ...
        <button className="btn btn-success"><a href="/dashboard/lecturer-register" style={{textDecoration:'none', color:'white'}}>Add New Lecturer</a></button>
        &nbsp;
        &nbsp;
        &nbsp;
        &nbsp;
        <button className="btn btn-success"><a href="/dashboard/lecturer-requests" style={{textDecoration:'none', color:'white'}}>Generate Report</a></button>
        &nbsp;
        &nbsp;
        &nbsp;
        &nbsp;
        <button className="btn btn-success"><a href="/dashboard/request" style={{textDecoration:'none', color:'white'}}>Request Table</a></button>
    </div>
  )
}
}