...
export default class AllStudents extends Component{
  ...
  retrieveInstructors(){
    axios.get("http://localhost:8070/students").then(res =>{
      if(res.data.success){
        this.setState({
          instructors:res.data.existingStudents
        });
  ...
  onDelete = (id) =>{
    axios.delete(`http://localhost:8070/students/delete/${id}`).then((res) =>{
      ...
      <div className="container">
        <div className="row">
          <div className="col-lg-9 mt-2 mb-2">
            <h4 className='header-name'>All students</h4>
          </div>
          ...
          <tbody>
            {this.state.instructors.map((instructors,index) =>(
              <tr key={index}>
                ...
                <td>
                  <a href={`/dashboard/lecturer/${instructors._id}`} style={{textDecoration:'none'}}>
                ...
                <td>
                  <a className="btn btn-warning" href={`/dashboard/edit-student/${instructors._id}`}>
        ...
        <button className="btn btn-success"><a href="/dashboard/students-register" style={{textDecoration:'none', color:'white'}}>Add New Student</a></button>
        &nbsp;
        &nbsp;
        &nbsp;
        &nbsp;
        <button className="btn btn-success"><a href="/dashboard/reportgenerate-student" style={{textDecoration:'none', color:'white'}}>Generate Report</a></button>
        &nbsp;
        &nbsp;
        &nbsp;
        &nbsp;
        <button className="btn btn-success"><a href="/dashboard/students-requests" style={{textDecoration:'none', color:'white'}}>Request Table</a></button>
    </div>
  )
}
}