...
export default class RequestStudent extends Component{
    constructor(props){
        super(props);
    ...
    retrieveLecturers(){
        axios.get("http://localhost:8070/students").then(res =>{
            if(res.data.success){
                this.setState({
                    lecturers:res.data.existingStudents
                });
            ...
            <div className="container">
                <div className="row">
                    <div className="col-lg-9 mt-2 mb-2">
                        <h4 className='header-name'>All students Requests</h4>
                    </div>
                        ...
                        {this.state.lecturers.map((lecturers,index) =>(
                            <tr key={index}>
                                <th scope="row">{index+1}</th>
                                    ...
                                    <td>
                                        <a className="btn btn-warning" href={`/dashboard/edit-student/${lecturers._id}`}>
